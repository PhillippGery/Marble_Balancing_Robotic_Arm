...
- *Plate angular velocity:* Computed via a Jacobian-based approach using 100 Hz joint state data. Each column of the rotational Jacobian is the Z-axis of the corresponding joint frame in the world frame, obtained from TF. The dot product `J_rot @ q_dot` gives the plate's angular velocity vector without directly differentiating the noisy Euler angles.
