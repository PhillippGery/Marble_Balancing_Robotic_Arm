"""
train_cmaes_gazebo.py
---------------------
CMA-ES validator for the TD3 residual policy.

Optimises a linear policy (28 → 2) using CMA-ES and compares its best
...
Usage (from rl_training/ directory):
  python train_cmaes_gazebo.py                         # offline (fast)
  python train_cmaes_gazebo.py --no-offline            # Gazebo (slow!)
  python train_cmaes_gazebo.py --td3-model models_td3/best_model_td3.zip
  python train_cmaes_gazebo.py --stage 2 --max-iters 200
...
OBS_DIM    = 28
ACTION_DIM = 2
N_PARAMS   = OBS_DIM * ACTION_DIM + ACTION_DIM   # 58


def policy_linear(params: np.ndarray, obs: np.ndarray) -> np.ndarray:
    """Linear map: (28,) → (2,) with tanh squash to [-1, 1]."""
...
def train(args):
    ...
    if args.offline:
        ...
        rclpy.init()
        env = GazeboRLEnv(stage=args.stage)

    # ── TD3 baseline ──────────────────────────────────────────────────────────
    _here      = os.path.dirname(os.path.abspath(__file__))
    models_dir = os.path.join(_here, 'models_td3')
    td3_path   = args.td3_model or os.path.join(models_dir, 'best_model_td3.zip')
    norm_path  = args.td3_norm  or os.path.join(models_dir, 'running_stats.pkl')

    n_td3_eps = 20 if args.offline else 5
    td3_reward = evaluate_td3(env, td3_path, norm_path, n_eps=n_td3_eps)

    # ── CMA-ES ────────────────────────────────────────────────────────────────
    out_dir = os.path.join(_here, 'models_cmaes')
    ...
    while not es.stop():
        ...
        if ep_best > best_reward:
            ...
            save_path   = os.path.join(out_dir, 'best_params.pkl')
    ...
    print(f'CMA-ES params saved to: {os.path.join(out_dir, "best_params.pkl")}')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='CMA-ES validator for TD3 residual policy')
    ...
    parser.add_argument('--td3-model',  type=str,   default='',
                        help='Path to TD3 model .zip for comparison')
    parser.add_argument('--td3-norm',   type=str,   default='',
                        help='Path to running_stats.pkl for TD3 normalisation')
